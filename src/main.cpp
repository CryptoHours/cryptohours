...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 0.8 * FACTOR * COIN;
    } else if (nHeight <= 12*YEAR && nHeight >= 8*YEAR) {
        nSubsidy = 0.6 * FACTOR * COIN;
    } else if (nHeight <= 16*YEAR && nHeight >= 12*YEAR) {
        nSubsidy = 0.4 * FACTOR * COIN;
