...
bool AppInit(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("CryptoHours Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n";

        if (mapArgs.count("-version")) {
            strUsage += LicenseInfo();
        } else {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  pivxd [options]                     " + _("Start CryptoHours Core Daemon") + "\n";
    ...
    try {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "cryptohours:"))
                fCommandLine = true;
        ...
        if (fDaemon) {
            fprintf(stdout, "CryptoHours server starting\n");
