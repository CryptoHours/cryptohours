...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CryptoHours";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cryptohours.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cryptohoursd.pid"));
